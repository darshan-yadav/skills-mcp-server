...
async def execute_tool(
    ...
    try:
        ...
        raise ExecutorError(f"Script {script_rel_path} not found in bundle.")
    ...
    try:
        ...
        raise ExecutorError(f"Failed to spawn subprocess: {e}")
    ...
    try:
        ...
        return ToolResult(
            isError=True,
            content=[ToolResultContent(type="text", text=f"Execution timed out after {timeout} seconds.")]
        )
    ...
    if process.returncode != 0:
        return ToolResult(
            ...
            content=[ToolResultContent(
                type="text", 
                text=f"Process exited with code {process.returncode}. Stderr: {stderr.decode('utf-8', errors='replace')}"
            )]
        )
    ...
    try:
        ...
        return ToolResult(
            ...
            content=[ToolResultContent(
                type="text", 
                text=f"Failed to parse script output as JSON: {e}\\nRaw output: {stdout_str}"
            )]
        )
    except Exception as e:
        return ToolResult(
